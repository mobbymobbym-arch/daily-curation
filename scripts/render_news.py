...
def render_deep_analysis(data):
    ...
    if isinstance(data, dict):
        for key in ['Stratechery', 'Dwarkesh', 'stratechery', 'dwarkesh']:
            if key in data and data[key]:
                item = data[key]
                item['source_key'] = key
                analysis_items.append(item)
        
        # Also check for other nested items if not caught above
        if not analysis_items:
            for source, content in data.items():
                if isinstance(content, dict) and ('title' in content or 'title_zh' in content):
